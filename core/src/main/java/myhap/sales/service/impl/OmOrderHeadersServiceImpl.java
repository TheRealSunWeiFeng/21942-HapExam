...
import myhap.sales.mapper.OmOrderHeadersMapper;
import myhap.sales.service.*;
...
public class OmOrderHeadersServiceImpl extends BaseServiceImpl<OmOrderHeaders> implements IOmOrderHeadersService{
    ...
    public static final String SALES_LINEHEADER_ID = "SALES_LINEHEADER_ID";
    public static final String SALES_LINE_ID = "SALES_LINE_ID";
    @Autowired
    ...
    public List<OmOrderHeaders> saveLinesAndHeader(IRequest requestContext, List<OmOrderHeaders> dto, int page, int pageSize) throws TaskExecuteException {
        ...
        try {
            for (OmOrderHeaders orderHeader : dto) {
                if (orderHeader.getHeaderId() == null || orderHeader.getHeaderId() == 0) {
                    //新建头行逻辑
                  //  String headerCode = codeRuleProcessService.getRuleCode(SALES_LINEHEADER_ID);//SALES_LINEHEADER_ID

                    String ran = 100000 + new Random().nextInt(900000)+"";
                    String headerCode = Calendar.getInstance().getTimeInMillis()+ran +"";//用时间戳代替
                    headerCode = headerCode.substring(8);
                    ...
                    if (orderLines != null) {
                        for (OmOrderLines orderLine : orderLines) {
                            ...
                            //String lineCode = codeRuleProcessService.getRuleCode(SALES_LINE_ID);

                            String ran1 = 100000 + new Random().nextInt(900000)+"";
                            String lineCode = Calendar.getInstance().getTimeInMillis()+ran1 +"";//用时间戳代替

                            lineCode = lineCode.substring(8);
        ...
        } catch (Exception e) { //CodeRuleException e
            e.printStackTrace();
    ...
    @Override
    public SXSSFWorkbook buildExportOrderExcel(IRequest requestContext, OmOrderHeaders dto, int page, int pageSize) {
        ...
        List<OmOrderHeaders> orders = selectWithForgeinKey(requestContext, dto, page, pageSize);
